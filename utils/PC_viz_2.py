...
def shift_point_cloud(pcd, translation):
    def callback(vis):
        pcd.translate(translation, relative=True)
        return False
    return callback

def visualize_with_interaction(pcd1, pcd2):
    vis = o3d.visualization.VisualizerWithKeyCallback()
    vis.create_window()
    vis.add_geometry(pcd1)
    vis.add_geometry(pcd2)

    # Register key callbacks for moving pcd2
    vis.register_key_callback(ord('X'), shift_point_cloud(pcd2, [0.1, 0, 0]))  # Move along X
    vis.register_key_callback(ord('Y'), shift_point_cloud(pcd2, [0, 0.1, 0]))  # Move along Y
    vis.register_key_callback(ord('Z'), shift_point_cloud(pcd2, [0, 0, 0.1]))  # Move along Z
...
def merge(dataset: ModelParams, opt: OptimizationParams, pipe: PipelineParams, testing_iterations, saving_iterations, 
    ...
    o3d.visualization.draw_geometries([down_pcd1, down_pcd2, coordinate_frame])
