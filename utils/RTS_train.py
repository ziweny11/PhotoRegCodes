...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint: str, checkpoint2: str, debug_from, 
    ...
    best_param = (scale, rot, g2scale)

    lr_list = [0.001, 0.0001, 0.001]

    threshold = 0.3

    #set up extrinsincs cam dicts:
    path1 = 'playground/wr1'
    ...
    cam_RT_list1 = filtercam(g1, g2copy, cam_extrinsics1, dataset, pipe, threshold = 0.7, 
    ...
    for epoch in range(20000):
        ...
        if epoch == 0:
            ...
            scale, rot, g2scale = best_param

        lr_list =  [x * 1.0 for x in lr_list]
        
        optimizer = Adam([
        {'params': [scale], 'lr': lr_list[0]},
...
if __name__ == "__main__":
    ...
    R2tmp = np.array([[0.7531,  0.0573, -0.6554], 
                      [-0.1673,  0.9801, -0.1065], 
                      [0.6363,  0.1898,  0.7477]]
