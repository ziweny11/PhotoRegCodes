...
def filtercam(g1, g2copy, extrinsics, dataset, pipe, threshold = 0.1, sim_threshold = 0.6, flag = 1):
    ...
    for key in list(extrinsics.keys()):
        try:
            ...
            cam_pos_right = myMiniCam3(979,652, rotmat, tvec, fovx, fovy, znear, zfar)

            cam_R_new = (RI1).T @ rotmat 
            cam_T_new = tvec - (TI1) @ (RI1).T @ rotmat
            cam_pos_new = myMiniCam3(979,652, cam_R_new, cam_T_new, fovx, fovy, znear, zfar)
            ...
            if c1 and c2 and c3 and img.name != "DSC01019.JPG":
                print(img.name)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint: str, checkpoint2: str, debug_from, 
    ...
    print(g2._xyz.shape)
    #ini
    scale = torch.tensor([18.0], requires_grad=True, device='cuda')
    g2scale = torch.tensor([1.94], requires_grad=True, device='cuda')
    ...
    lr_list = [0.0001, 0.0001, 0.00001]

    threshold = 0.3

    #set up extrinsincs cam dicts:
    path1 = 'playground/multi2/m2_2'
    ...
    cam_RT_list1 = filtercam(g1, g2copy, cam_extrinsics1, dataset, pipe, threshold = 0.4, 
                             sim_threshold = 0.7, flag = 1)
    ...
    for epoch in range(20000):
        ...
        for iter in range(700):
            ...
            cam_pos_right2 = myMiniCam3(979,652, cam_R_right2, cam_T_right2, fovx, fovy, znear, zfar)
            ...
            cam_pos_new2 = myMiniCam3(979,652, cam_R_new3, cam_T_new3, fovx, fovy, znear, zfar)
            ...
            cam_pos_right = myMiniCam3(979,652, cam_R_right, cam_T_right, fovx, fovy, znear, zfar)
...
#          [ 0.2304, -0.0213,  0.9729, -0.0319]
    ...
    R2tmp = np.array( [[ 0.8213, -0.4876,  0.2963], 
                       [0.5097,  0.8603,  0.0028], 
                       [-0.2563,  0.1488,  0.9551]])
