...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint: str, checkpoint2: str, debug_from, 
    ...
    g2scale = torch.tensor([0.85], requires_grad=True, device='cuda')
    ...
    cam_RT_list1 = filtercam(g1, g2copy, cam_extrinsics1, dataset, pipe, threshold = 0.3, 
                             sim_threshold = 0.6, flag = 1)
    ...
    for epoch in range(20000):
        ...
        # Select two elements
        sample = cam_RT_list1[index:index + 1]
        index += 1
        cam_R_right, cam_T_right = sample[0]
...
if __name__ == "__main__":
    ...
    RI1tmp = np.array([[ 0.4038,  0.3048, -0.8626],
        [-0.1667,  0.9516,  0.2583],
        [ 0.8995,  0.0395,  0.4351]])
    RI1 = torch.tensor(RI1tmp, dtype=torch.float32, device="cuda")
    TI1tmp = np.array([-1.8906,  1.3673, -2.1584])
    ...
    RI2tmp =  np.array([[ 0.9940, -0.0471,  0.0991],
        [ 0.0517,  0.9977, -0.0443],
        [-0.0967,  0.0492,  0.9941]])
    RI2 = torch.tensor(RI2tmp, dtype=torch.float32, device="cuda")
    TI2tmp = np.array([-4.7641,  0.5992,  0.1404])
...
#          [ 0.2304, -0.0213,  0.9729, -0.0319]
    R2tmp = np.array( [[ 0.9975,  0.0265, -0.0657], 
                       [-0.0243,  0.9991,  0.0345], 
                       [ 0.0666, -0.0328,  0.9972]]).T
    T2tmp = np.array([-0.0333, 0.0137,-0.0032]) @ R2tmp
