...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint: str, checkpoint2: str, debug_from, 
    ...
    #ini
    scale = torch.tensor([13.0], requires_grad=True, device='cuda')
    ...
    for epoch in range(200):
        ...
        lr_list =  [x * 0.9 for x in lr_list]
        ...
        for iter in range(300):
            ...
            # render2_new = render(cam_pos_new2, gnew, pipe, background, scaling_modifer)
            ...
            render_new = render(cam_pos_new, gnew, pipe, background, scaling_modifer)

            img_new = render_new["render"]
            mask_new = render_new["mask"]
            with torch.no_grad():
                # if measure_blurriness(img_new) > 0.03:
                #     break
