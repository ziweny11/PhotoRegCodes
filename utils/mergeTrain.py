...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint: str, checkpoint2: str, debug_from, 
    ...
    for epoch in range(200):
        ...
        for iter in range(10000):
            ...
            cam_R_new3 = (RI2).T @ cam_R_right2
            cam_T_new3 = cam_T_right2 - (TI2) @ ((R2).T) @ cam_R_right2

            cam_pos_new2 = myMiniCam3(450, 250, cam_R_new3, cam_T_new3, fovx, fovy, znear, zfar)
            render2_new = render(cam_pos_new2, gnew, pipe, background, scaling_modifer)["render"]
            torch.cuda.empty_cache()
            ...
            with torch.no_grad():
                if measure_blurriness(img_new2) > 0.05:
                    torch.cuda.empty_cache()
                    break
                render2_ref = render(cam_pos_right2, g2, pipe, background, scaling_modifer)["render"]
                torch.cuda.empty_cache()
                img_ref2 = render2_ref
