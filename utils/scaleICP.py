...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint: str, checkpoint2: str, debug_from, 
    ...
    gaus_copy(g1, g1copy)
    gaus_copy(g2, g2copy)
    sc = torch.tensor([0.6], dtype=torch.float32, device="cuda")
    ...
    source_pcd = tensor_to_pcd(pc1)
    target_pcd = tensor_to_pcd(pc2_align)

    red_color = np.array([[1, 0, 0] for _ in range(len(pc1))])
    blue_color = np.array([[0, 0, 1] for _ in range(len(pc2))])
    source_pcd.colors = o3d.utility.Vector3dVector(red_color)
    target_pcd.colors = o3d.utility.Vector3dVector(blue_color)
    ...
    # Perform ICP
    threshold = 0.1  # set a suitable threshold for your application
