...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint: str, checkpoint2: str, debug_from, 
    ...
    #ini
    scale_start, scale_end, scale_step = 10, 20, 0.1
    g2scale_start, g2scale_end, g2scale_step = 0.7, 1.3, 0.01
    scales = np.arange(scale_start, scale_end + scale_step, scale_step)
    g2scales = np.arange(g2scale_start, g2scale_end + g2scale_step, g2scale_step)
    param_list = [(float(scale), float(g2scale)) for scale in scales for g2scale in g2scales]
    ...
    offset = torch.tensor([0.0, 0.0, 0.0], requires_grad=False, device='cuda')
    rot = matrix_to_quaternion(R2)
    rot.to(dtype=torch.float32, device="cuda")
    rot.requires_grad_(False)

    min_loss = 100
    cnt = 0
    for s, gs in param_list:
        cnt += 1
        torch.cuda.empty_cache()
        scale = torch.tensor([s], requires_grad=False, device='cuda')
        g2scale = torch.tensor([gs], requires_grad=False, device='cuda')
